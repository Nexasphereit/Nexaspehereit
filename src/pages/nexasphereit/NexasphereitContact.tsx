...
export default function NexasphereitContact() {
  ...
  const submitContact = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const savedLeads = JSON.parse(localStorage.getItem('nexasphereit_leads_backup') || '[]');
      savedLeads.push({ ...form, createdAt: new Date().toISOString(), status: 'New', id: 'local_' + Date.now() });
      localStorage.setItem('nexasphereit_leads_backup', JSON.stringify(savedLeads));
      setSubmitted(true);
    } finally {
      setLoading(false);
    }
