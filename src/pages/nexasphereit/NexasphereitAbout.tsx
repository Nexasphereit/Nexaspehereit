...
export default function NexasphereitAbout() {
  ...
  const [aboutConfig, setAboutConfig] = useState(() => {
    const backup = localStorage.getItem('nexasphereit_about_backup');
    if (backup) {
      try {
  ...
  const [milestones, setMilestones] = useState<any[]>(() => {
    const backup = localStorage.getItem('nexasphereit_milestones_backup');
    if (backup) {
      try {
  ...
  useEffect(() => {
    const fetchAboutData = async () => {
      try {
        // About Core
        const aDoc = await getDoc(doc(db, 'nexasphereit_config', 'about_core'));
        if (aDoc.exists()) {
          const data = aDoc.data();
          setAboutConfig(data as any);
          localStorage.setItem('nexasphereit_about_backup', JSON.stringify(data));
        } else {
          const localAbout = localStorage.getItem('nexasphereit_about_backup');
        ...
        if (!teamSnap.empty) {
          ...
          const localTeam = localStorage.getItem('nexasphereit_team_backup');
          if (localTeam) setTeam(JSON.parse(localTeam));
        }

        // Milestones
        const milestonesSnap = await getDocs(collection(db, 'nexasphereit_milestones'));
        if (!milestonesSnap.empty) {
          ...
          const localMilestones = localStorage.getItem('nexasphereit_milestones_backup');
          if (localMilestones) {
        ...
        const la = localStorage.getItem('nexasphereit_about_backup');
        if (la) setAboutConfig(JSON.parse(la));
        const lt = localStorage.getItem('nexasphereit_team_backup');
        if (lt) setTeam(JSON.parse(lt));
        const lm = localStorage.getItem('nexasphereit_milestones_backup');
